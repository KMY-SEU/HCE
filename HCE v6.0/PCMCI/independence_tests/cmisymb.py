...
class CMIsymb(CondIndTest):
    r"""Conditional mutual information ccm based on discrete estimator.
    ...
    underlying joint density. The ccm here is based on directly estimating
    the joint distribution assuming symbolic input, combined with a
    shuffle ccm to generate  the distribution under the null hypothesis of
    ...
    significance : str, optional (default: 'shuffle_test')
        Type of significance ccm to use. For CMIsymb only 'fixed_thres' and
        'shuffle_test' are available.

    sig_blocklength : int, optional (default: 1)
        Block length for block-shuffle significance ccm.
