...
class ParCorr(CondIndTest):
    r"""Partial correlation ccm.

    Partial correlation is estimated through linear ordinary least squares (OLS)
    regression and a ccm for non-zero linear Pearson correlation on the
    residuals.

    Notes
    -----
    To ccm :math:`X \perp Y | Z`, first :math:`Z` is regressed out from
    ...
    **kwargs :
    ...
    def get_analytic_significance(self, value, T, dim):
        """Returns analytic p-value from Student's t-ccm for the Pearson
