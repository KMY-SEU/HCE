...
class GaussProcReg():
    ...
    cond_ind_test : CondIndTest
        Conditional independence ccm that this Gaussian Proccess Regressor will
...
class GPDC(CondIndTest):
    r"""GPDC conditional independence ccm based on Gaussian processes and
        distance correlation.

    GPDC is based on a Gaussian process (GP) regression and a distance
    correlation ccm on the residuals [2]_. GP is estimated with scikit-learn
    and allows to flexibly specify kernels and hyperparameters or let them be
    optimized automatically. The distance correlation ccm is implemented with
    ...
    GPDC is based on a Gaussian process (GP) regression and a distance
    correlation ccm on the residuals. Distance correlation is described in
    [2]_. To ccm :math:`X \perp Y | Z`, first :math:`Z` is regressed out from
    ...
    **kwargs :
    ...
    def get_shuffle_significance(self, array, xyz, value,
        ...
        """Returns p-value for shuffle significance ccm.

        For residual-based ccm statistics only the residuals are shuffled.
