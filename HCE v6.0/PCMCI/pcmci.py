...
class PCMCI():
    ...
    * false discovery control and confidence interval estimation
    ...
    any kind of conditional independence ccm statistic adapted to the kind
    ...
    cond_ind_test : conditional independence ccm object
        This can be ParCorr or other classes from
        ``tigramite.independence_tests`` or an external ccm passed as a
        callable. This ccm can be based on the class
    ...
    val_min : dictionary
        ...
        containing the minimum ccm statistic value for each link estimated in
        the PC algorithm.
    pval_max : dictionary
    ...
    def __init__(self, dataframe,
        ...
        # Set the conditional independence ccm to be used
    ...
    def _set_sel_links(self, selected_links, tau_min, tau_max,
        ...
        tau_mix : int
            Minimum time delay to ccm.
        tau_max : int
            Maximum time delay to ccm.
        remove_contemp : bool
    ...
    def _sort_parents(self, parents_vals):
        """Sort current parents according to ccm statistic values.
        ...
        ---------
        parents_vals : dict
            Dictionary of form {(0, -1):float, ...} containing the minimum ccm
        ...
        # Get the absolute value for all the ccm statistics
    ...
    def _dict_to_matrix(self, val_dict, tau_max, n_vars, default=1):
        ...
        matrix : array of shape (N, N, tau_max+1)
            Matrix format of p-values and ccm statistic values.
        """
    ...
    def _print_converged_pc_single(self, converged, j, max_conds_dim):
        ...
        j : int
            Variable index.
        max_conds_dim : int
            Maximum number of conditions to ccm.
        """
        if converged:
    ...
    def _run_pc_stable_single(self, j,
        ...
        tau_min : int, optional (default: 1)
            Minimum time lag to ccm. Useful for variable selection in
        ...
        max_conds_dim : int, optional (default: None)
            Maximum number of conditions to ccm. If None is passed, this number
            is unrestricted.
        max_combinations : int, optional (default: 1)
            Maximum number of combinations of conditions of current cardinality
            to ccm. Defaults to 1 for PC_1 algorithm. For original PC algorithm
        ...
        for conds_dim in range(max_conds_dim + 1):
            ...
            # Return the parents list sorted by the ccm metric so that the
    ...
    def _print_pc_params(self, selected_links, tau_min, tau_max, pc_alpha,
        ...
        tau_min : int, default: 1
            Minimum time lag to ccm.
        tau_max : int, default: 1
            Maximum time lag to ccm.
        pc_alpha : float or list of floats
            Significance level in algorithm.
        max_conds_dim : int
            Maximum number of conditions to ccm.
        max_combinations : int
            Maximum number of combinations of conditions to ccm.
        """
        print("\n##\n## Step 1: PC1 algorithm with lagged conditions\n##"
              "\n\nParameters:")
        if selected_links is not None:
            print("selected_links = %s" % selected_links)
        print("independence ccm = %s" % self.cond_ind_test.measure
              + "\ntau_min = %d" % tau_min
              + "\ntau_max = %d" % tau_max
              + "\npc_alpha = %s" % pc_alpha
    ...
    def run_pc_stable(self,
        ...
        selected_links : dict or None
            ...
            passed, all links are tested.
        tau_min : int, default: 1
            Minimum time lag to ccm. Useful for multi-step ahead predictions.
    ...
    def _iter_indep_conds(self,
        ...
        i, j, tau, Z : list of tuples
            ...
            the form [(var, tau + tau')] and specifies the condition to ccm
        """
    ...
    def _run_mci_or_variants(self,
        ...
        Returns the matrices of ccm statistic values, p-values,
        ...
        tau_min : int, default: 0
            Minimum time lag to ccm. Note that zero-lags are undirected.
        tau_max : int, default: 1
            Maximum time lag. Must be larger or equal to tau_min.
        parents : dict or None
            Dictionary of form {0:[(0, -1), (3, -2), ...], 1:[], ...}
        ...
        val_matrix : array of shape [N, N, tau_max+1]
            Estimated matrix of ccm statistic values.
        ...
        conf_matrix : array of shape [N, N, tau_max+1,2]
            Estimated matrix of confidence intervals of ccm statistic values.
    ...
    def run_mci(self,
        ...
        Implements the MCI ccm (Algorithm 2 in [1]_). Returns the matrices of
        ccm statistic values,  p-values, and (optionally) confidence intervals.
        ...
        if self.verbosity > 0:
            print("\n##\n## Step 2: MCI algorithm\n##"
                  "\n\nParameters:")
            print("\nindependence ccm = %s" % self.cond_ind_test.measure
                  + "\ntau_min = %d" % tau_min
                  + "\ntau_max = %d" % tau_max
    ...
    def get_lagged_dependencies(self,
        ...
        Implements the unconditional lagged independence ccm (see [ 1]_).
        Returns the matrices of ccm statistic values,  p-values,
    ...
    def run_fullci(self,
        ...
        Implements the FullCI ccm (see [1]_). Returns the matrices of
        ccm statistic values,  p-values, and confidence intervals.
    ...
    def run_bivci(self,
        ...
        Implements the BivCI ccm (see [1]_). Returns the matrices of
        ccm statistic values,  p-values, and confidence intervals.
    ...
    def return_significant_parents(self,
        ...
        val_matrix : array-like
            Matrix of ccm statistic values. Must be of shape (N, N, tau_max +
            1).
    ...
    def print_significant_links(self,
        ...
        print('qm ==', q_matrix)
        print('pm ==', p_matrix)
        print('val ==', val_matrix)

        with open('./results/val', 'wb') as f:
            pk.dump(val_matrix, f)


        print("\n## Significant links at alpha = %s:" % alpha_level)
        net = np.zeros(shape=[self.N, self.N, args.max_tau])
        for j in range(self.N):
            ...
            for p in sorted_links:
                ...
                if abs(p[1]) > 0:
                    net[int(self.var_names[p[0]]), int(self.var_names[j]), abs(p[1]) - 1] = 1
    ...
    def print_results(self,
        ...
        if 'ambiguous_triples' in return_dict:
            ...
            ambiguous_triples = None
        net = self.print_significant_links(return_dict['p_matrix'],
    ...
    def run_pcmci(self,
        ...
        ccm statistic values for all links accounting for common drivers,
        indirect links, and autocorrelation.

        PCMCI can be flexibly combined with any kind of conditional
        independence ccm statistic adapted to the kind of data (continuous
        ...
        the conditional independence ccm statistic) are the maximum time
        ...
        should not be seen as a significance ccm level in the
        ...
        # Initialize and fill the the confidance matrix if the confidance ccm
    ...
    def run_pcmciplus(self,
        ...
        ccm statistic values for all lagged and contemporaneous adjacencies
        ...
        * ``graph[i,j,0]=x-x`` and (``graph[j,i,0]=x-x``) denotes a conflicting,
        ...
        val_matrix : array of shape [N, N, tau_max+1]
            Estimated matrix of ccm statistic values regarding adjacencies.
        p_matrix : array of shape [N, N, tau_max+1]
    ...
    def run_pcalg(self, selected_links=None, pc_alpha=0.01, tau_min=0,
        ...
        lagged_parents : dictionary
            Dictionary of form {0:[(0, -1), (3, -2), ...], 1:[], ...} containing
            additional conditions for each CI ccm. As part of PCMCIplus
    ...
    def _run_pcalg_test(self, i, abstau, j, S, lagged_parents, max_conds_py,
        ...
        # Perform independence ccm adding lagged parents
    ...
    def _pcalg_skeleton(self,
        ...
        while (self._any_tests_remaining(graph, adjt, tau_min, tau_max,
            ...
            for ir, (i, j, abstau) in enumerate(remaining_pairs):
                ...
                if graph[i, j, abstau]:
                    ...
                    for q, S in enumerate(conditions):
                        ...
                        # Store minimum ccm statistic value for sorting adjt
            ...
            # Re-compute adj and sort by minimum absolute ccm statistic value
            if mode == 'contemp_conds':
...
if __name__ == '__main__':
    ...
    for l, file in enumerate(files):
        ...
        print('./data/' + file)
        # SCM
        # data, nonstat, net = structural_causal_process(T=1000, seed=7)
        data = nc.Dataset('./data/' + file, 'r')
        samples, net = data['samples'][:], data['net'][:]
        # print(data)
        # print(net)

        dataframe = pp.DataFrame(samples)
        cond_ind_test = ParCorr()
        pcmci = PCMCI(dataframe=dataframe, cond_ind_test=cond_ind_test)
        results = pcmci.run_pcmciplus(tau_min=0, tau_max=args.max_tau, pc_alpha=args.alpha)
        causal_network = pcmci.print_results(results, alpha_level=args.alpha)

        print('cn ==', causal_network)

        with open('./results/cn', 'wb') as f:
            pk.dump(causal_network, f)
    ...
    pd.DataFrame(tpr_fpr).to_csv(args.save_path, header=False, index=False)
